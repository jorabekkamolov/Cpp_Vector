...
test:
	mkdir -p $(BUILD_DIR)
	$(CXX) $(CXXFLAGS) $(TEST) $(LDFLAGS) -o $(BUILD_DIR)/vector
...
valgrind:
	mkdir -p $(VAL_DIR)
	$(CXX) $(CXXFLAGS) $(TEST) $(VAFLAGS) -o $(VAL_DIR)/vector
	$(VALGRIND) $(VALGRINDFLAGS) ./$(VAL_DIR)/vector

run:
	./$(BUILD_DIR)/vector

coverage: $(BUILD_DIR)/vector
	mkdir -p $(COV_DIR)
	./$(BUILD_DIR)/vector
